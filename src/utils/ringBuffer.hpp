...
// https://loosechainsaw.github.io/c++/2020/02/19/ringbuffer/
...
namespace detail {
	...
	class ring_buffer_iterator {
		using buffer_t = typename std::conditional<!C, RingBuffer<T, N, Overwrite>*, RingBuffer<T, N, Overwrite> const*>::type;
	public:
...
class RingBuffer {
	using self_type = RingBuffer<T, N, Overwrite>;
public:
	...
	RingBuffer(RingBuffer const& rhs) noexcept(std::is_nothrow_copy_constructible_v<value_type>)
	{
		copy_impl(rhs, std::bool_constant<std::is_trivially_copyable_v<T>>{});
	}
	RingBuffer& operator=(RingBuffer const& rhs) noexcept(std::is_nothrow_copy_constructible_v<value_type>) {
		if(this == &rhs)
			return *this;
